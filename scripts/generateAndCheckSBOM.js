...
async function main() {
  ...
  if (!cmd.quick) {
    ...
    await run('mvn -ntp -B clean install -T 1C -q -DskipTests -Dvaadin.npm.minimumFrontendPackageAgeDays=0');
  }
  ...
  if (!cmd.quick) {
    ...
    await run('mvn clean package -ntp -B -Pproduction -DskipTests -q -Dvaadin.npm.minimumFrontendPackageAgeDays=0');
