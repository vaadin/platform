  ...
  ret.components.forEach(c => {
    c.licenses && c.licenses.forEach(l => {
      if (/vaadin/.test(c.purl)) {
        ...
        if (l.license && /raw\.githubusercontent\.com\/vaadin\/.*LICENSE/.test(l.license.name || l.license.url || '')) {
          l.license = { id: VAADIN_LICENSE };
        }
      }
...
async function getReleases() {
  return (await run(`git ls-remote --tags origin`, { debug: false }))
    .stdout.split('\n').map(l => l.replace(/.*refs\/tags\//, '')).filter(l => /^(1[4-9]|[2-9]\d)\.\d+\.\d/.test(l));
}

async function getBranches() {
  return (await run(`git branch -r`, { debug: false }))
    .stdout.split('\n').map(b => b.replace(/ *origin\//, '')).filter(l => /^(1[4-9]|[2-9]\d)\.\d+$/.test(l));
}
...
async function main() {
  ...
  // The sbom module is standalone and takes the platform version through -Drevision, so
  // every artifact is resolved from the repositories. Building the platform is neither
  // needed nor wanted here: released versions are immutable and already published.
  const mvnRev = `-Drevision=${currVersion}`;
  ...
  if (!cmd.quick) {
    ...
    await run(`mvn clean package -ntp -B -Pproduction -DskipTests -q ${mvnRev} -Dvaadin.npm.minimumFrontendPackageAgeDays=0`);
    await run(`mvn dependency:tree -ntp -B ${mvnRev}`, { output: 'target/tree-maven.txt' });
    await run(`mvn -ntp -B org.cyclonedx:cyclonedx-maven-plugin:makeAggregateBom -q ${mvnRev}`);
    await run('npm ls --depth 6', { output: 'target/tree-npm.txt' , throw: false});
    await run('npm install --ignore-scripts --silent');
    await run('npx --yes @cyclonedx/cyclonedx-npm --ignore-npm-errors --output-file target/bom-npm.json --output-format JSON');
  }
  ...
  if (cmd.useOWASP) {
    ...
    !cmd.quick && await run(`mvn org.owasp:dependency-check-maven:12.2.2:check ${mvnRev} ${owaspArgs}`, { throw: false });
