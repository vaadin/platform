...
async function getRequest(repo, pullNumber, auth){
	url='https://api.github.com/repos/vaadin/'+repo+'/pulls/'+pullNumber+'/comments'
...
async function getRequestIssue(repo, pullNumber, auth2){
	url='https://api.github.com/repos/vaadin/'+repo+'/issues/'+pullNumber+'/comments'
	let res = await axios.get(url, {
		headers: {
			'User-Agent': 'flowinfo',
...
for(t=0;t<repoList.length;t++){

console.log("processing "+repoList[t] );
const csvWriter = createCsvWriter({
    path: repoList[t]+'-github-info.csv',
    header: [
	...
	console.log("Reading the pull number file for "+repoList[t] );
	pulls = fs.readFileSync('pull_number/'+repoList[t]+'.txt').toString().split("\n");
	//console.log(pulls);
	//pulls = [100, 1000, 8765];
	for(j=0;j<pulls.length-1;j++){
		//pulls.length
		let records = [];
		console.log("Request data from Github for pull request " +pulls[j]+ " in "+repoList[t] );
	    data = await getRequest(repoList[t],pulls[j],auth);
		...
		console.log("Request data from Github for issues " +pulls[j]+ " in "+repoList[t] );
		issueData = await getRequestIssue(repoList[t],pulls[j],auth2);
		...
		await sleep(5000);
		console.log("Writting records in "+repoList[t]+"-github-info.csv"+ " (finished "+ (j+1)/(pulls.length-1)*100 +"%)");
